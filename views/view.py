#toda a logica da aplicacao (classe de negócios)
...
class SubscriptionService:
    ...
    def delete(self, id):
        with Session(self.engine) as session:
            ...
            obj.status=0
            session.add(obj)
            session.query(Payments).filter(Payments.subscription_id == id).delete(synchronize_session="fetch")
            session.commit()
    ...
    def _get_last_12_months_native(self):
        ...
        return last_12_months[::-1] #devolve na ordem reversa
    ...
    def gen_chart(self):
        ...
        last_12_months = list(map(lambda x: f"{x[0]:02d}/{x[1]}" , self._get_last_12_months_native()))
